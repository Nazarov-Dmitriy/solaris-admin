...
<template><form class="flex gap-3 min-h-max justify-center">
            <SelectButton v-model="selectedRole" :options="roles" optionLabel="label" optionValue="value" />
        </form>
    <div class="min-h-screen w-full flex items-center justify-center bg-gray-100">
        
        <form @submit.prevent="() => createCompetition(competitionFields)"
            ...
            <div class="space-y-4">
                <div v-for="(field, index) in formFields" :key="index" class="space-y-2">
                    ...
                    <component :is="field.component" v-model="competitionFields[field.model]" :id="field.id"
                        ...
                        :style="field.style" :formats="field.formats" v-tooltip.top="field.tooltip"
                        :options="field.options" :optionLabel="field.optionLabel" :optionValue="field.optionValue" />
                </div>
                <div class="my-4">
                    <Button type="submit" class="w-full">Обновить</Button>
                </div>
            </div>
        </form>
    </div>
...
import { CreateFormField, Roles } from '../../../features/competitions/types';
import Editor from 'primevue/editor';
import { DatePicker, Select, SelectButton } from 'primevue';
...
const roles: Roles[] = [
    { value: 'updateCompetition', label: 'Обновить конкурс' },
    { value: 'addNastavnik', label: 'Добавить наставников в конкурс' },
];

const selectedRole = ref<string>('createCompetition');
...
const competitions = computed(() => competitionStore.getCompetition);
const competition = computed(() => competitions.value.find(e => e.id === Number(route.params.id)));

watch(competition, (newCompetition) => {
    if (newCompetition) {
        competitionFields.value = { ...newCompetition };
    }
}, { immediate: true });
